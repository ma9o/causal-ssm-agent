...
export default function Stage5bContent({ code, data }: { code: string; data: Stage5bData }) {
  const [activeData, setActiveData] = useState(data);
  const mock = isMockMode();

  return (
    <div className="space-y-4">
      {mock && <MockMethodSwitcher code={code} baseData={data} onDataChange={setActiveData} />}
