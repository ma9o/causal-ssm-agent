...
const GRAPH: Stage4Graph = {
  nodes: [
    ...SMALLGOLDEN_INDICATORS.map((name) => ({
      ...
      label: `Likelihood: ${titleize(name)}`,
      phase: "model_decisions",
    })),
    {
      id: "loading:screen_time",
      kind: "loading_decision",
      label: "Loading Constraints: Screen Time",
      phase: "model_decisions",
    },
    ...
    },
    {
      ...
      label: "Measurement Priors: Screen Time",
      phase: "prior_blocks",
    },
    ...SMALLGOLDEN_DYNAMICS.map((name) => ({
      id: `dynamics:${name}`,
      kind: "dynamics_prior",
      label: `Dynamics: ${titleize(name)}`,
      phase: "prior_blocks",
    })),
    ...SMALLGOLDEN_EFFECT_TARGETS.map((name) => ({
      id: `effects:${name}`,
      kind: "effect_prior",
      label: `Incoming Effects: ${titleize(name)}`,
      phase: "prior_blocks",
    })),
    ...SMALLGOLDEN_CORRELATIONS.map((name) => ({
      id: `correlation:${name}`,
      kind: "correlation_prior",
      label: `Initial Correlation: ${correlationLabel(name)}`,
      phase: "prior_blocks",
    })),
    {
