/**
 * Ideal materialized analysis artifact for development & stories.
 *
 * Real persisted analysis traces in the seed workspace (DEMO) carry
 ...
 * workspace would store it. Driving the viewer through `buildBaselineReportScenarios`
...
import demoBaselineReport from "../../__fixtures__/demo-run/baseline_report.json";
...
const demo = demoBaselineReport as unknown as BaselineReportData;
  ...
  messages: [
    {
      role: "system",
      content:
        "You are exploring follow-up composable scenarios for analysis of a fitted causal state-space model of affective state.",
      tool_is_error: false,
    },
    {
      role: "assistant",
...
/** The complete ideal materialized analysis artifact (baselines + sims + summary). */
