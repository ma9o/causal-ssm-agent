...
export function TrajectoryCard({
  ...
  return (
    <g opacity={isPrev ? 0.42 : 1}>
      ...
      {latent ? (
        <path d={area(refLo, refHi)} fill={MUTED} fillOpacity={0.16} stroke="none" />
      ) : null}
      {latent && moved ? (
        <path d={area(lo, hi)} fill={TEAL} fillOpacity={0.16} stroke="none" />
      ) : null}
      ...
      {latent && realized
        ? realized
            ...
            .map((v, t) => (
              <circle
                key={t}
                cx={sx(days[t]).toFixed(1)}
                cy={sy(v).toFixed(1)}
                r={1.3}
                fill={DAG_COLORS.realized}
                fillOpacity={0.75}
              />
            ))
        : null}
      ...
      {hov != null ? (
        <g pointerEvents="none">
          ...
          <circle
            ...
            cy={sy(pkey[hov] ?? 0)}
            r={2.8}
            fill={moved ? TEAL : latent ? SLATE : MUTED}
            stroke="#fff"
            strokeWidth={1}
          />
          {(() => {
