...
describe("triggerRun", () => {
  ...
  it("returns flow run id", async () => {
    ...
    const id = await triggerRun("dep-123", { code: "test" });
  ...
  it("sends parameters in request body", async () => {
    ...
    await triggerRun("dep-123", { code: "test", query: "test.txt" });

    expect(apiFetch).toHaveBeenCalledWith(
      "/prefect/deployments/dep-123/create_flow_run",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ parameters: { code: "test", query: "test.txt" } }),
      }),
    );
  });
  ...
  it("propagates API errors from apiFetch", async () => {
    ...
    await expect(triggerRun("dep-123", { code: "test" })).rejects.toThrow("API error 500");
  });
});
