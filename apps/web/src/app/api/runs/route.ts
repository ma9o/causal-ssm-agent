...
function buildInitialRunIdempotencyKey(workspaceId: string, launchId: string): string {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!deploymentId) {
      return NextResponse.json({ error: "causal-inference deployment not found" }, { status: 502 });
    }
    ...
    const existingFlowRunId = await findFlowRunIdByIdempotencyKey(deploymentId, idempotencyKey);
