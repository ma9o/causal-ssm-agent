"""Auxiliary cSMC sampler: blocked aux-cSMC latent + MALA parameter."""
...
def _pathfinder_init_positions(
    ...
    *,
    ...
    if not bool(jax.device_get(jnp.all(jnp.isfinite(state.position)))):
        raise RuntimeError("Pathfinder returned a non-finite mode for aux_csmc init.")
    if not bool(jax.device_get(jnp.isfinite(state.elbo))):
        raise RuntimeError("Pathfinder returned a non-finite ELBO for aux_csmc init.")
    ...
    if not bool(jax.device_get(jnp.all(jnp.isfinite(positions)))):
        raise RuntimeError("Pathfinder returned non-finite chain-init positions for aux_csmc.")
    diagnostics = {
...
def fit_aux_csmc(
    ...
    **_kwargs,
    ...
    if parameter_kernel != "mala":
        raise ValueError(
            f"Unsupported aux_csmc parameter kernel {parameter_kernel!r}. Supported: 'mala'."
        )
    if init_method not in {"random", "pathfinder"}:
        raise ValueError(
            f"Unsupported aux_csmc init_method {init_method!r}. "
