"""Particle latent samplers: auxiliary cSMC / Particle-mGRAD + MALA parameter."""
...
def _fit_particle_latent_mcmc(
    ...
    *,
    ...
    reparam,
    gradient_informed: bool | None = None,
) -> InferenceResult:
    ...
    if latent_kernel == "csmc":
        latent_kernel_spec = build_auxiliary_csmc_latent_kernel(
            bundle,
            delta=latent_delta,
            ...
            gradient_informed=bool(gradient_informed),
        )
    elif latent_kernel == "particle_mgrad":
        ...
        raise ValueError(
            f"Unsupported {method_name} latent kernel {latent_kernel!r}. "
            "Supported: 'csmc' or 'particle_mgrad'."
        )
    ...
    if gradient_informed is not None:
        kernel_diagnostics["gradient_informed"] = gradient_informed
...
def fit_aux_csmc(
    ...
    **_kwargs,
    ...
    """Fit an SSM with blocked auxiliary cSMC/MALA complete-data MCMC.

    ``initial_positions_override``:
        Optional ``(num_chains, dim)`` array of unconstrained chain starts.
        When provided it overrides ``init_method`` entirely — use this to
        supply custom initialisations such as mixing Pathfinder-seeded and
        random-perturbation chains (different parameters benefit from
        different inits; see ``build_pathfinder_random_mixed_init_positions``).
    """
    if latent_kernel != "csmc":
        raise ValueError(
            f"Unsupported aux_csmc latent kernel {latent_kernel!r}. Supported: 'csmc'."
        )
    return _fit_particle_latent_mcmc(
        model,
        observations,
        times,
        method_name="aux_csmc",
        diagnostics_key="aux_csmc",
        latent_kernel="csmc",
        num_particles=n_csmc_particles,
        particle_count_diagnostic_key="n_csmc_particles",
        num_warmup=num_warmup,
        num_samples=num_samples,
        num_chains=num_chains,
        seed=seed,
