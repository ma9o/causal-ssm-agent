...
def build_auxiliary_csmc_latent_kernel(
    ...
    *,
    ...
    ``gradient_informed`` enables the §3.3.1 differentiable-model proposal: the
    conditioning target ``u_t`` is shifted by ``(δ_t/2) ∂gamma/∂u_t`` (Langevin
    drift under the observation log-likelihood), with the matching
    ``(x_t - u_t) · ∂gamma/∂u_t`` importance correction on the particle weights.
    Paper predicts this reduces cSMC bias in heterogeneously-informative
    observation regimes without increasing per-step complexity beyond one
    extra gradient evaluation of the observation log-likelihood.
    """
    if num_particles < 2:
        raise ValueError("aux_csmc requires num_particles >= 2.")

    obs_increment_fn = bundle["observation_increment_log_prob_from_context_fn"]
    observation_grad_from_context_fn = bundle["observation_grad_from_context_fn"]
    ...
    def _latent_csmc_step(state, key: jnp.ndarray):
        ...
        if gradient_informed:
            # ∂gamma/∂u_t evaluated at the current auxiliary draw — drives the
            # §3.3.1 Langevin shift of the conditioning target.
            grad_u = jnp.asarray(
                observation_grad_from_context_fn(context, u),
                dtype=latent_dtype,
            )
            u_cond = u + auxiliary_var * grad_u
        else:
            grad_u = jnp.zeros_like(u)
            u_cond = u

        init_mean, init_cov = _initial_latent_moments(context)
        gain0, _conditioned_cov0, chol0, innovation_cov0 = _condition_gaussian_on_auxiliary(
            init_cov,
            jnp.diag(auxiliary_var[0]),
        )
        init_guided_mean = init_mean + (u_cond[0] - init_mean) @ gain0.T
        ...
        log_w0 = log_w0 + _gaussian_log_prob_full(u_cond[0], init_mean, innovation_cov0)
        if gradient_informed:
            # Importance correction from the ratio N(x;u,sigma^2 I) / N(x; u + sigma^2 grad, sigma^2 I)
            # in the proposal swap. Up to a particle-independent constant this
            # is -(x - u) . grad (see Corenflos & Sarkka eq. 37 for sigma^2 = delta/2).
            log_w0 = log_w0 - jnp.sum((x0_particles - u[0][None, :]) * grad_u[0][None, :], axis=-1)
        ...
        if num_steps == 1:
            ...
            def _forward_step(carry, inputs):
                ...
                gain_t, _conditioned_cov_t, chol_t, innovation_cov_t = (
                    _condition_gaussian_on_auxiliary(
                        context.Qd[time_idx],
                        jnp.diag(auxiliary_var[time_idx]),
                    )
                )
                u_cond_t = u_cond[time_idx]
                prop_mean_free = pred_free + (u_cond_t - pred_free) @ gain_t.T
                eps_t = random.normal(
                    propagate_key,
                ...
                aux_norm = jax.vmap(
                    lambda mean: _gaussian_log_prob_full(u_cond_t, mean, innovation_cov_t)
                )(pred_all)
                ...
                if gradient_informed:
                    # See the matching correction on log_w0 above — same sign.
                    log_w = log_w - jnp.sum(
                        (x_particles - u[time_idx][None, :]) * grad_u[time_idx][None, :],
                        axis=-1,
                    )
