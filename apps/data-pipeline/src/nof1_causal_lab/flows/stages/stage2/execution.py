"""Stage 2 execution helpers for semantic worker batches (asyncio-native)."""
...
async def collect_worker_results(
    *,
    ...
    logger.info("Stage 2: waiting for %d worker results", batch_total)
    ...
    while pending:
        ...
        for task in done:
            ...
            try:
                ...
                logger.warning(
                    "Stage 2: worker %d failed (batch=%d/%d, windows=%d): %s",
                    worker_id,
                    batch_completed,
                    batch_total,
                    n_windows,
            ...
            logger.info(
                "Stage 2: worker %d %s (batch=%d/%d, windows=%d, extractions=%d, output_rows=%d)",
                worker_id,
                status,
                batch_completed,
                batch_total,
    ...
    logger.info("Stage 2: collected %d/%d workers", len(statuses_by_worker), total_chunks)
