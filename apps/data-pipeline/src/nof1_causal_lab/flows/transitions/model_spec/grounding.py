...
from nof1_causal_lab.flows.transitions.model_spec.agentic.feedback import (
    ModelSpecGroundingResult,
    make_model_spec_grounding_result,
)
from nof1_causal_lab.flows.transitions.model_spec.agentic.text import summarize_model_spec_names

# ---------------------------------------------------------------------------
# model-spec: Model grounding (statistical model spec + priors, unified)
# ---------------------------------------------------------------------------


def model_spec_grounding(
    data: dict,
    causal_design: dict,
    ...
    *,
    ...
    The function merges proposals with ``current`` (existing model-spec state),
    ...
    if new_statistical_model_spec is not None and new_statistical_model_spec == state.get(
        ...
        return make_model_spec_grounding_result(
            context_output=None,
            status="update_rejected",
            feedback=_format_redundant_model_spec_update_feedback(
                "model decisions",
    ...
    if isinstance(new_priors, dict):
        ...
        if not new_priors:
            return make_model_spec_grounding_result(
                context_output=None,
                status="update_rejected",
                feedback=_format_redundant_model_spec_update_feedback("priors", redundant_priors),
                state_retained=True,
                retain_for_next_prompt=True,
                capture_context_output=False,
            )
    ...
    if validation.compiled_ssm is None:
        raise ValueError("model-spec grounding requires compiled_ssm before resolving priors")
...
def should_capture_model_spec_output(result: ModelSpecGroundingResult) -> bool:
    """Return whether a model-spec tool result should become the new accepted state.
    ...
    the last accepted state, or the final materialized model-spec payload can drift
    away from what the tool actually accepted.
    """
    return result.context_output is not None and result.validation_packet.capture_context_output
...
def _format_missing_priors_feedback(missing_priors: list[str]) -> str:
    ...
    return (
        "MODEL STATE SAVED:\n"
        "- missing priors for "
        f"{len(missing_priors)} parameters: {summarize_model_spec_names(missing_priors)}\n"
...
def _format_redundant_model_spec_update_feedback(kind: str, names: list[str]) -> str:
    """Tell the LLM not to resend already accepted model-spec fields."""
    summary = summarize_model_spec_names(names)
...
def _with_stateful_retry_guidance(feedback: str) -> str:
    """Remind the LLM that accepted model-spec state is preserved across retries."""
    return f"{feedback}\n\nPreviously accepted state is retained. Resubmit only the fields you changed."
