...
def merge_priors(existing: dict[str, dict] | None, new: dict[str, dict] | None) -> dict[str, dict]:
    """Merge prior updates into the current model-spec state."""
...
def format_prior_proposal_errors(errors: dict[str, str]) -> str:
    """Render prior schema errors in the user-facing model-spec format."""
    blocks = []
...
def materialize_model_spec_result(
    *,
    ...
    """Build the full grounded model-spec result from authored inputs.
