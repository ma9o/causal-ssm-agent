...
def make_context_tool(
    ...
    On success (context_output is not None), the capture dict is updated.
    ...
    Args:
        ...
        compute_fn: (data_dict) -> (context_output | None, feedback_str)
    ...
    async def _execute(**kwargs: str) -> str:
        ...
        should_capture = (
            capture_when(context_output, feedback) if capture_when else context_output is not None
        )
        if should_capture and context_output is not None:
            capture.update(context_output)
        if is_success:
