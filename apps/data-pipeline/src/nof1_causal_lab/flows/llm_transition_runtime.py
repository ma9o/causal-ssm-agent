"""Shared lifecycle helpers for LLM-backed transitions."""
...
class LLMTransitionRuntimeConfig:
    """Factory settings for one LLM-backed transition."""
...
def build_scoped_session_factory(config: LLMTransitionRuntimeConfig) -> ScopedSessionFactory:
    """Create the context-scoped session factory used throughout one LLM transition."""
...
def attach_trace(payload: dict[str, Any], trace: LLMTrace) -> dict[str, Any]:
    """Attach a serialized trace when the transition generated one."""
...
@asynccontextmanager
async def open_llm_transition(
    *,
    config: LLMTransitionRuntimeConfig,
    logger: logging.Logger,
) -> AsyncIterator[ScopedSessionFactory]:
    """Open the context session factory for one stage run."""
    started_at = time.monotonic()
    logger.info("[%s] starting", config.context_id)
    factory = build_scoped_session_factory(config)
    try:
        yield factory
        ...
        if trace_messages:
            ...
            logger.error("[%s] failed after %.1fs: %s", config.context_id, elapsed, exc)
        raise
    elapsed = time.monotonic() - started_at
    logger.info(
        ...
        config.context_id,
        elapsed,
        config.profile_llm.harness,
        config.profile_llm.model,
    )


def make_llm_transition_runner(
    *,
    context_id: str,
    orchestrator_fn: Callable[..., Awaitable[Any]],
    profile_llm_getter: Callable[[], LLMProfileConfig],
    ...
    """Build a plain async runner for an LLM-backed transition.

    The wrapper opens a :class:`ScopedSessionFactory` bound to the stage's
