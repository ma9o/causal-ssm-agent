"""Compare particle_mgrad against MAP + IEKS on the 10-latent mixed-support recovery.

Usage:
    uv run python scripts/run_particle_mgrad_vs_map_recovery.py
"""
...
def _run_one(method: str, data: dict[str, Any], **fit_kwargs: Any) -> dict[str, Any]:
    ...
    if method == "particle_mgrad":
        sampler_diag = {
            k: v
            for k, v in result.diagnostics.get("particle_mgrad", {}).items()
...
def main() -> None:
    ...
    parser.add_argument("--n-particles", type=int, default=25)
    parser.add_argument(
        "--backward-sampling",
    ...
    print(
        "[compare] running method=particle_mgrad (joint (x, theta) blocked Gibbs with Particle-mGRAD)...",
        flush=True,
    )
    particle_run = _run_one(
        "particle_mgrad",
        data,
    ...
    print(f"[compare] particle_mgrad done in {particle_run['elapsed_seconds']:.1f}s")
    print(f"[particle_mgrad diag] {particle_run['sampler_diag']}")

    print()
    print(f"{'family':<16}{'metric':<12}{'particle_mgrad':>12}{'map':>12}")
    print("-" * 52)
    for family in ("drift", "diffusion_sd", "obs_scale", "obs_df"):
        for metric in ("mean_abs_error", "mean_ci_width", "coverage"):
            av = particle_run["summary"][family][metric]
            mv = map_run["summary"][family][metric]
            print(f"{family:<16}{metric:<12}{av:>12.4f}{mv:>12.4f}")

    print()
    payload = {"particle_mgrad": particle_run, "map": map_run}
