"""Run the 10-latent mixed-support MAP recovery benchmark.

Local:
    uv run python scripts/run_map_mixed_support_recovery.py

Modal:
    uv run modal run scripts/run_map_mixed_support_recovery.py --gpu A100
"""
...
def _make_mixed_support_recovery_data(*, n_time: int) -> dict[str, Any]:
    ...
    prior_realized_drift_mean = 0.35
    base_decay_prior_mean = prior_realized_drift_mean - spec.stability_margin
    base_decay_prior_sd = 0.15
    priors = SSMPriors(
        drift_base_decay={
            "family": get_positive_runtime_family_index(PriorDistributionFamily.GAMMA),
            "concentration": (base_decay_prior_mean / base_decay_prior_sd) ** 2,
            "rate": base_decay_prior_mean / base_decay_prior_sd**2,
        },
...
def _summarize_family_recovery(
    ...
    families = [
        (
            "drift",
            -(samples["drift_base_decay_free"] + data["spec"].stability_margin),
...
def run_benchmark(config: dict[str, Any], *, check: bool) -> dict[str, Any]:
    ...
    diagnostics = {
        ...
        "optimizer": result.diagnostics.get("optimizer"),
        "optimizer_success": result.diagnostics.get("optimizer_success"),
        "optimizer_nit": result.diagnostics.get("optimizer_nit"),
        "mode_log_posterior": result.diagnostics.get("mode_log_posterior"),
        "mode_log_likelihood": result.diagnostics.get("mode_log_likelihood"),
    ...
    failures = _evaluate_thresholds(summary)
...
def main() -> None:
    ...
    parser.add_argument("--maxiter", type=int, default=DEFAULT_CONFIG["maxiter"])
    parser.add_argument(
        "--progress-bar",
