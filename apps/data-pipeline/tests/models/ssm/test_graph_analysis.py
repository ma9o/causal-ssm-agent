...
class TestSelectDefaultMethod:
    def test_non_point_observation_support_routes_to_aux_gibbs(self):
        """Interval-summary observations should route to aux_gibbs."""
        spec = _make_spec()
        ...
        assert select_default_method(spec, observation_support=support) == "aux_gibbs"

    def test_gaussian_model_routes_to_aux_gibbs(self):
        """Fully Gaussian model with identity links → aux_gibbs."""
        spec = _make_spec(
            n_latent=2,
            n_manifest=2,
            lambda_mat=jnp.eye(2),
    ...
    def test_poisson_obs_routes_to_aux_gibbs(self):
        """Poisson observations → aux_gibbs."""
        spec = _make_spec(
    ...
    def test_explicit_kalman_override_routes_to_aux_gibbs(self):
        """Explicit likelihood override should preserve the default aux_gibbs method."""
        spec = _make_spec(
            manifest_dists=[DistributionFamily.POISSON, DistributionFamily.POISSON],
            manifest_links=[LinkFunction.LOG, LinkFunction.LOG],
        )
        assert select_default_method(spec, likelihood="kalman") == "aux_gibbs"
...
class TestPlanInferenceStructure:
    ...
    def test_per_channel_mixed_routes_to_aux_gibbs(self):
        """Per-channel mixed distributions: one Poisson channel → aux_gibbs."""
        spec = _make_spec(
            n_latent=2,
            n_manifest=2,
            lambda_mat=jnp.eye(2),
    ...
    def test_large_gaussian_model_routes_to_aux_gibbs(self):
        """Larger fully Gaussian model → aux_gibbs."""
        n = 5
        spec = _make_spec(
            n_latent=n,
            n_manifest=n,
    ...
    def test_zero_dep_gaussian_channel_still_routes_aux_gibbs(self):
        """Gaussian zero-dep channel doesn't break pure-Kalman routing → aux_gibbs."""
        H = jnp.array(
            [
                [1.0, 0.0],
                [0.0, 1.0],
    ...
    def test_zero_dep_nongaussian_channel_routes_to_aux_gibbs(self):
        """Non-Gaussian zero-dep channel prevents pure-Kalman → aux_gibbs."""
        H = jnp.array(
            [
                [1.0, 0.0],
                [0.0, 1.0],
