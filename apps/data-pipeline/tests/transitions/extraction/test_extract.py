...
def test_extract_window_chunk_uses_stage2_generate_config(monkeypatch, caplog):
    ...
    extraction_llm = LLMProfileConfig(harness="none", model="openrouter/mock-stage2-model")
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            extraction_workers=SimpleNamespace(
                llm=extraction_llm,
                max_tool_turns=55,
                worker_timeout=120,
            ),
        ...
        result = _run(
            stage2_extract.extract_window_chunk(
                ...
                measurement_structure={
                    "model_clock": "1d",
                    "indicators": [{"name": "indicator_a"}, {"name": "indicator_b"}],
                },
            )
        )
    ...
    assert worker_kwargs["measurement_structure"] == {
        "model_clock": "1d",
        "indicators": [{"name": "indicator_a"}, {"name": "indicator_b"}],
    }
...
def test_extract_window_chunk_emits_running_stage2_worker_and_snapshot_events(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            extraction_workers=SimpleNamespace(
                llm=LLMProfileConfig(harness="none", model="openrouter/mock-stage2-model"),
                max_tool_turns=40,
                worker_timeout=120,
            ),
            llm=LLMDefaults(
    ...
    _run(
        stage2_extract.extract_window_chunk(
            ...
            chunk_idx=7,
            question="Q",
            measurement_structure={"model_clock": "1d", "indicators": [{"name": "indicator_a"}]},
            workspace_id="ws-123",
        )
    )
...
def test_run_measurements_extraction_core_accepts_injected_semantic_chunk_runner(
    monkeypatch,
):
    ...
    result = _run(
        stage2_extract.run_extraction_core(
            raw_df=raw_df,
            question="Does stress affect sleep?",
            measurement_structure=causal_design["measurement"],
            extraction_workers=SimpleNamespace(
                windows_per_chunk=8,
    ...
    assert [_require_mapping(ctx)["indicators"][0]["name"] for ctx in chunk_contexts] == [
        "stress_score",
        "monthly_sleep_hours",
    ]
...
def test_run_semantic_chunks_asyncio_emits_extraction_plan_worker_and_snapshot_events(monkeypatch):
...
def _extraction_workers_config() -> SimpleNamespace:
    return SimpleNamespace(
        extraction_workers=SimpleNamespace(
            windows_per_chunk=8,
            max_events_per_window=50,
...
def test_run_measurements_extraction_buckets_semantic_indicators_by_observation_window(monkeypatch):
    import nof1_causal_lab.utils.config as config_mod
    import nof1_causal_lab.utils.data as data_mod
    import nof1_causal_lab.workers.windows as windows_mod
    ...
    monkeypatch.setattr(config_mod, "get_config", _extraction_workers_config)
    ...
    result = _run(
        stage2_extract.run_extraction(
            raw_df,
            "Does stress affect sleep?",
            causal_design["measurement"],
        )
    )
    ...
    assert [
        captured_contexts[idx]["indicators"][0]["name"] for idx in sorted(captured_contexts)
    ] == [
        "stress_score",
        "monthly_sleep_hours",
    ]
...
def test_run_measurements_extraction_annotates_medical_imaging_monthly_summary_support_window(
    monkeypatch,
):
    import nof1_causal_lab.utils.config as config_mod
    import nof1_causal_lab.workers.windows as windows_mod
...
def test_run_measurements_extraction_annotates_semantic_rows_into_canonical_observation_rows(
    monkeypatch,
):
    import nof1_causal_lab.utils.config as config_mod
    import nof1_causal_lab.workers.windows as windows_mod
...
def test_run_measurements_extraction_merges_computed_rule_rows_with_semantic_rows(monkeypatch):
    import nof1_causal_lab.utils.config as config_mod
    import nof1_causal_lab.workers.windows as windows_mod
