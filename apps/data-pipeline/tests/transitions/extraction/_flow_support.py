"""Test-owned support for the retired asyncio extraction entrypoints."""
...
from tests.transitions.extraction._execution import (
    collect_worker_results,
    run_with_retries,
)
from tests.transitions.extraction._progress import (
...
class ScopedSessionFactory:
    """Test-owned stage session factory for retired extraction entrypoints."""

    def __init__(self, profile_llm, llm_defaults, *, context_id, max_tool_turns=None):
        from nof1_causal_lab.utils.llm import LLMTrace

        del llm_defaults, context_id
...
async def extract_window_chunk(
    ...
    from tests.transitions.extraction._worker import run_worker_extraction
...
async def _run_semantic_chunks_asyncio(
    *,
    ...
    if max_rpm:
        openrouter_client._limiters["llm"] = openrouter_client.RpmLimiter(max_rpm)
    ...
    try:
        ...
        openrouter_client._limiters.pop("llm", None)
