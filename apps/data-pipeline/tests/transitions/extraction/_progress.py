"""Test support for retired asyncio extraction progress tracking."""
...
def emit_extraction_snapshot(workspace_id: str, snapshot: dict[str, int]) -> None:
    ...
    limiter = openrouter_client._limiters.get("llm")
    request_count = len(limiter._timestamps) if limiter is not None else 0

    emit_extraction_snapshot_event(
        workspace_id,
        snapshot={
            **snapshot,
            "llm_requests_last_60s": request_count,
        },
    )
