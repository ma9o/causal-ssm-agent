"""Inspect AI evaluation for orchestrator models on the MEDICAL_SEMANTICS fixture.

Ranks competing orchestrator models on how well they reproduce the fixed
MEDICAL_SEMANTICS Stage 2 fixture after running the real Stage 1a -> 1b -> 2
domain components against a fixed Stage 0 parquet and fixed worker model.

Usage:
    inspect eval evals/multi_model/eval_medical_semantics_orchestrator.py
    inspect eval evals/multi_model/eval_medical_semantics_orchestrator.py --model openrouter/anthropic/claude-opus-4.6
    inspect eval evals/multi_model/eval_medical_semantics_orchestrator.py -T models=openrouter/anthropic/claude-opus-4.6,openrouter/openai/gpt-5.1
"""
...
from causal_ssm_agent.flows.stages.stage2_extract import (
    materialize_stage2_outputs,
...
Each candidate used:
- the same fixed MEDICAL_SEMANTICS Stage 0 parquet and question
...
1. `stage1b_surface`: indicator identity, support semantics, and window choices
2. `stage2_structure`: row shape, coverage, support bounds, and per-indicator counts
...
def create_eval_dataset() -> MemoryDataset:
    """Create the fixed MEDICAL_SEMANTICS eval dataset."""
    fixture = load_medical_semantics_fixture()
    return MemoryDataset(
        [
            Sample(
                input=fixture.question,
                id="medical_semantics",
                metadata={
...
def medical_semantics_ranking_scorer():
