# Stage 1b: Measurement Structure and Identifiability

| Modality | Interactive | Produces |
|---|---|---|
| Semantic | Yes | [`CausalDesign`](#causaldesign), [`IdentificationReport`](#identificationreport) when positive |

Operationalizes the [`LatentStructure`](01a-latent-structure.md#latent-structure) against observed data by specifying indicators for each construct, then checks whether each treatment-to-outcome effect is causally identifiable[^pearl2009].
...
| `latent_structure` | [Stage 1a](01a-latent-structure.md) | `LatentStructure` with constructs and edges |
...
`CausalDesign` is the combined input to downstream stages that includes the Stage 1a latent structure, the Stage 1b measurement structure, and the derived identifiability status:

| Field | Type | Description |
|---|---|---|
| `latent` | [`LatentStructure`](01a-latent-structure.md#latent-structure) | The validated Stage 1a construct-level graph |
