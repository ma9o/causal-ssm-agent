...
Maps constructs to observable indicators and checks whether each treatment-outcome effect is causally identifiable.
...
| Interactive | Yes |
| Gate | Hard gate |
| Produces | `CausalSpec` plus identifiability status |
...
3. Validate three things together:
   ...
   - causal identifiability via y0's ID algorithm
...
| `MeasurementModel` | `{indicators, model_clock}` | `model_clock` is the observation-window width used for extraction and discretization |
