...
`support_kind`, `summary_operator`, and `anchor_policy` are derived deterministically from each indicator's `aggregation` and `measurement_dtype` via the [observation semantics](../primitives/measurement-model/windows-and-aggregation.md). They are not free parameters—the measurement model fully determines them.
