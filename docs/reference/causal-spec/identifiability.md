...
**Rationale:** Under A1, A6, and A9 the construct covariance matrix is identified from observed indicator data. Pearl-style[^pearl2009] identification criteria then apply to the construct-level DAG as if constructs were directly observed. This two-step logic—identify the measurement model, then identify the causal model—follows Anderson and Gerbing (1988)[^anderson1988] and is one reason `CausalSpec`, not `LatentModel` alone, is the right handoff object for downstream fitting. Proxy indicators for latent confounders follow Miao, Geng, and Tchetgen Tchetgen (2018)[^miao2018].
