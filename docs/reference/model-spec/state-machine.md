...
This page explains the [Stage 4](../../pipeline/04-model-specification-priors.md) control loop at a high level. It is meant to answer questions like "what is the reducer trying to do?", "what can reopen?", and "why does the stage sometimes pause for validation before asking the LLM another question?"

For the exact block-by-block contract, see [LLM-Driven Stage 4 Specification](llm-driven-specification.md). For the artifact contract that downstream stages consume, see [Stage 4](../../pipeline/04-model-specification-priors.md).
...
3. lock those decisions into one executable `ModelSpec`,
...
| `review:model_spec` | Check the locked model form as a whole before prior authoring starts. |
...
| Draft model decisions | Holds accepted model-form choices before the full `ModelSpec` is locked. |
| Accepted artifacts | Holds the locked `ModelSpec`, accepted priors, and latest accepted validation result. |
...
| Settling mode | The reducer runs deterministic follow-on work such as locking the `ModelSpec`, selecting the next block, or validating a finished repair scope. |
...
First, `review:model_spec` is part of the nominal path. It is a checkpoint, not an error state.
...
The transition from model decisions to prior authoring is not direct. Stage 4 first pauses to lock the `ModelSpec`.
...
"Given the accepted indicator choices and model-level switches, is there a valid `ModelSpec` to build on?"
...
| Accepted locked state | The accepted `ModelSpec`, the accumulated accepted priors, and the latest accepted validation result. |
...
| Checkpoint | Job |
|---|---|
| `review:model_spec` | Ask whether the locked model-form decisions still make sense when viewed together. It can reopen model-decision blocks, but it does not author priors. |
...
| `review:model_spec` | Reopened implicitly when model-form repair makes the joint checkpoint relevant again. |
...
| Condition | Meaning |
|---|---|
| Locked model form exists | The reducer has an accepted `ModelSpec`. |
